...
ns.Changelog = {
	{
		...
		sections = {
			{ head = "New Features", items = {
				"A lane can now stay up while a cooldown is still running on it. With Auto-hide Frames on, a lane's background, border, name and markers vanish the moment you leave combat, but your cooldown icons keep traveling, so an icon still counting down is left floating with no bar underneath it. Tick Keep Shown While Running under Lanes > a lane > General > Auto-hide and that lane holds its chrome until the last cooldown on it finishes, then hides as usual. Off by default, and set per lane, so you can give it to one lane and leave the others alone. Override Autohide still wins where you have ticked it, and the option stands aside while Unlock Frames is on so a lane never changes shape under your cursor mid-drag.",
				"Ready boxes can use your own background and border textures. Lanes and bars have had texture dropdowns since they shipped, while ready boxes quietly carried two texture settings of their own that nothing ever read, so a box was always drawn flat. Both work now, under Ready > a box > Appearance, and every LibSharedMedia texture you have installed is offered. The defaults are unchanged, so a box you have not touched looks exactly as it did.",
			} },
			{ head = "Bug Fixes", items = {
				"Switching profiles now moves cooldowns that are already running. Which lane and which bar a cooldown belongs to is worked out once, when it starts. A profile switch rebuilt everything around that without revisiting it, so anything already counting down stayed where the old profile had put it until it finished. A three minute cooldown sat in the wrong lane for three minutes, or disappeared entirely if the new profile sent it to a lane that is switched off. Show and hide updated instantly at the same moment, which is what made it read as a profile that had only half loaded. Importing a profile and switching spec with automatic profiles were both affected too.",
				"Border Texture set to None now draws no border. None is offered in every border texture list, but it resolves to nothing at all, which reads exactly like a texture that failed to load, so Cooldown Master fell back to its plain solid border and the setting looked dead. Unticking Show Border was the only way to actually remove one. None now means none, on lanes and on ready boxes alike.",
			} },
			{ head = "Improvements", items = {
				"Cooldown Master no longer asks the game to load Titan Panel or Bazooka. Both were listed as optional dependencies from an early version, which asks the game to load them at startup whether or not you use them. Neither needs it, they find Cooldown Master's minimap and broker data on their own, and a line of exactly this kind caused a real problem with another display addon back in 1.14.0. Masque is still listed, because it genuinely does have to load first.",
	...
	},
	{
		...
		sections = {
			{ head = "New Features", items = {
				"A bar frame can hand a cooldown over to a lane. A new Hand Off Below (sec) setting under Bars > a bar frame > General makes that frame let go once a cooldown has this many seconds left, so a lane carries it the rest of the way. The bar takes the long haul, the lane the final approach. Set it to a lane's Max Time, with that lane's Hide Long Timers ticked, and a ten minute cooldown sits on the bar until it reaches two minutes, leaves the bar, and starts traveling the lane. It applies to everything routed to that frame, not only to long cooldowns. 0, the default, keeps every cooldown on the bar right through to ready. Requested by dannyboy1262 on CurseForge.",
				"Bars can now show your very longest cooldowns. Ticking Show Extremely Long Cooldowns, on the same page, lets that one frame reach past the Ignore Threshold that keeps half-hour abilities off your lanes, up to 60 minutes. Everything shorter is covered too, so a 45 minute cooldown shows, not only the very longest ones. It applies to the frame you tick it on, so your lanes and ready boxes carry on as they were, and a cooldown you hid by hand on its Filters row stays hidden. A cooldown only a bar can see also finishes quietly rather than popping a ready box an hour after you cast it. Off by default. Also suggested by dannyboy1262.",
				"The two go together. Tick them on the frame your cooldowns actually go to, which is Default Bar under Filters, and remember an hour-long bar keeps its place in the list for the whole hour, so raise Max Bars or give it a frame of its own.",
			} },
			{ head = "Improvements", items = {
				"/cm cdv and /cm itemcd no longer contradict the screen. Both report whether a cooldown is visible, and with a bar frame showing extremely long cooldowns that answer differs per surface. They now name the bar frame that is allowed to draw it instead of flatly reporting it hidden.",
				"All seven languages are complete for this release, this feature included.",
			} },
		},
	},
