...
-- A hostile target's fields read secret in combat, and tostring throws on a secret, so unit reads route through here and blank out.
...
local TAGS = {
	...
	["player.hp"]       = function() return tostring(UnitHealth("player") or 0) end,
	["player.hp.pct"]   = function() return tostring(pct(UnitHealth("player"), UnitHealthMax("player"))) end,
	["player.power"]    = function() return tostring(UnitPower("player") or 0) end,
	["player.power.pct"]= function() return tostring(pct(UnitPower("player"), UnitPowerMax("player"))) end,
...
-- [cd.time] is Classic-only: retail secrets the value in combat, and the native countdown widget draws it anyway.
...
do
	...
	lines[#lines + 1] = "[player.hp.pct] / [player.power.pct]"
